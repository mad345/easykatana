#!/bin/sh
#Easy daemon setup for katana2x masternode
#by mad345
#simple usage, change max to your MN count. eg: max=4 for 4 MN setup
#this tool is very simple that can handle library for katana coin daemon, compile the daemon and prepare data directory for each masternode.
#yet you still need to copy and paste your masternodeprivkey=YOUR_GENKEY to each katana.conf
#here an example to start daemon
#./katanad -port=31954 -datadir=yourdatadir
#to make it easy, this tool create file rundaemon.sh
#call with : sh ./runmn.sh
#tested on ubuntu server 16.04 on virtual box

GIT_SOURCE_URL="https://github.com/getdatcrypto/k2x-source.git"
MASTERNODE=1
COIN_NAME="k2x"
CONF_FILE="katana.conf"
n=1
...
while [ "$n" -le "$max" ]; do
  ...
  echo addnode=24.21.56.209 >> $HOME/$DATADIR$n/$CONF_FILE
  if [ -z "$MASTERNODE" ]
    then
